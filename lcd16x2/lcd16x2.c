...
#include <stdio.h>
#include <stdarg.h>
#include  <stdbool.h> // ajoute Laurent avec les dernieres fonctions
...
static void lcd16x2_enablePulse(void)
{
  ...
  HAL_Delay(1); //DWT_Delay_us(T_CONST);
  HAL_GPIO_WritePin(PORT_RS_and_E, PIN_E, GPIO_PIN_RESET);
  HAL_Delay(1); //DWT_Delay_us(60);
}
...
// ajoute par Laurent pour pouvoir transformer int et float en string - recupere sur anciennes lib LCD
...
void lcd16x2_itoa(int n, char s[])
{
     int i, sign;
...
void lcd16x2_ftoa(float n, char *res, int afterpoint)
{
